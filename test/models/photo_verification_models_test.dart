/// Tests for PhotoVerificationData (Module 5 model)
library;
...
void main() {
  group('PhotoVerificationData defaults', () {
    test('has empty selections and feet as default unit', () {
      const data = PhotoVerificationData();
  ...
  group('PhotoVerificationData copyWith', () {
    test('copies fields without mutating the original', () {
      const original = PhotoVerificationData();
  ...
  group('PhotoVerificationData equality', () {
    test('two identical instances are equal', () {
      const a = PhotoVerificationData(
        heightCm: 170,
        heightUnit: HeightUnit.cm,
        languages: ['English'],
      );
      const b = PhotoVerificationData(
        heightCm: 170,
        heightUnit: HeightUnit.cm,
        languages: ['English'],
      );
    ...
    test('different values are not equal', () {
      const a = PhotoVerificationData(heightCm: 170);
      const b = PhotoVerificationData(heightCm: 171);
      expect(a, isNot(b));
    });
  });

  group('PhotoVerificationStep', () {
    test('isLast is true only for complete', () {
      expect(PhotoVerificationStep.height.isLast, isFalse);
      expect(PhotoVerificationStep.language.isLast, isFalse);
      expect(PhotoVerificationStep.photos.isLast, isFalse);
      expect(PhotoVerificationStep.complete.isLast, isTrue);
    });

    test('values are in declared order', () {
      expect(PhotoVerificationStep.values, [
